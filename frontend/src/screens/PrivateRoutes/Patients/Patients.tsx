...
export default function PatientList() {
  ...
  const getData = (params = {}) => {
    apiClient.get('patients/get', { params }).then(({ status, data }) => {
      if (status === 200) {
  ...
  const tableActions = (row: { id: any }) => (
    <div className="action-buttons">
      <ul>
        <li>
          <Link to={`/app/edit-industry/${row.id}`}>
  ...
  const columns = [
    {
      ...
      dataIndex: 'patientId',
      render: (v: string) => (
        <div onClick={() => navigate('/app/patients/add')}>
          <a>{v}</a>
        </div>
      )
    },
  ...
  return (
    <FilterLayout
      ...
      filter={<PatientFilter onSubmit={console.log} />}>
